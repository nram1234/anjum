  ...
  void onInit() {
    if (item.length == 0) {
      for (int i = 0; i < bata.allItems.length; i++) {
        item[bata.allItems[i].itemId] = TheItemInList(totalPriceForItem: 0,totalTaxForItem: 0,
                count: 0,
...
setTotalPriceAndTotalTex({String id }){
  item[id].value.totalTaxForItem=   item[id].value.afterdes*(item[id].value.tex/100);
  item[id].value.totalPriceForItem=    item[id].value.afterdes+item[id].value.totalTaxForItem;
  update();
}
 ...
 //   gettotalpriceincart();
  ...
  grandTotal.value=0;
  item.forEach((key, value) {
    grandTotal.value=grandTotal.value+(value.value.afterdes*value.value.count);
  ...
  getTotalTax(){
    totalTax.value=0;
    item.forEach((key, value) {
      totalTax.value=totalTax.value+(value.value.afterdes*value.value.tex);
